...
const equipmentTargets: Record<string, string> = {
  'interlock.cmps': 'CMPS',
  'interlock.ipps': 'IPPS',
...
function equipment(event: EventRecord) {
  return event.target ? equipmentTargets[event.target] ?? 'SYSTEM' : 'SYSTEM'
}
    ...
    <div v-else class="overflow-x-auto">
      <table class="w-full min-w-[760px] table-fixed text-left">
        <thead class="bg-slate-50 text-[9px] uppercase tracking-[0.12em] text-slate-400">
          <tr><th class="w-28 px-3 py-1.5">Time</th><th class="w-24 px-3 py-1.5">Severity</th><th class="w-28 px-3 py-1.5">Equipment</th><th class="w-[36%] px-3 py-1.5">Problem</th><th class="px-3 py-1.5">Details</th></tr>
        </thead>
        <tbody class="divide-y divide-slate-100 text-[11px]">
          <tr v-for="event in visible" :key="event.id" :class="event.severity === 'critical' ? 'border-l-2 border-l-red-500 bg-red-50/30' : 'border-l-2 border-l-amber-400 hover:bg-slate-50'">
