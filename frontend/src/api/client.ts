...
export const api = {
  ...
  getSession: (): Promise<SessionUser> => apiRequest('/session'),
  logout: (): Promise<void> => apiRequest('/logout', { method: 'POST' }),
  getTelemetry: (signal?: AbortSignal): Promise<TelemetryPoint> => apiRequest('/telemetry', { signal }),
  getSystemStatus: (signal?: AbortSignal): Promise<SystemStatus> => apiRequest('/status', { signal }),
  getEvents: (options: {
    limit?: number
    beforeId?: number
  ...
  updateUser: (username: string, role: UserRole): Promise<UsersResponse> =>
    apiRequest('/users/update', { method: 'POST', body: JSON.stringify({ username, role }) }),
  removeUser: (username: string): Promise<UsersResponse> =>
    apiRequest('/users/remove', { method: 'POST', body: JSON.stringify({ username }) })
}
